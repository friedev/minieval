...
func _input(event):
	...
	if event.is_action_pressed("info_overlay") && Recap.game_over == false && isPaused == false:
		...
		_set_currency_interactions("Bottom", buildings, "Cathedral", 16, "Forge", 14)
		_set_currency_interactions("Bottom", buildings, "Cathedral", 16, "Cathedral", 16)
		...
		_set_currency_interactions("Bottom", buildings, "Keep", 17, "Keep", 17)
